document.addEventListener("DOMContentLoaded", function () {
    ...
    const parentCurrencyDropdpwn = document.querySelector('#tabsForm');
    let windowWidth;
    function setCurrencyListPosition() {
        windowWidth = window.innerWidth;
    }
    setCurrencyListPosition()
    window.addEventListener('resize', setCurrencyListPosition)
    
    const $currencySelect2 = $(".js-example-basic-single").select2({
        dropdownParent: $(parentCurrencyDropdpwn),
        dropdownCssClass: windowWidth < 768 ? 'select2-dropdown--mobileCoordinates' : null,
    });

    $currencySelect2.on('select2:opening', function (e) {
        let dropdown = $('.select2-dropdown');
        dropdown.removeClass('select2-dropdown--mobileCoordinates'); // на всяк випадок
        if (window.innerWidth < 768) {
            dropdown.addClass('select2-dropdown--mobileCoordinates');
        }
    });
    ...
    function currencySwitch(country) {
        ...
        $currencySelect2.val(country).trigger("change");
        $('#select2-currency-container').text(country);
    ...
    //     setTimeout(() => {
    //       $dropdown.css({
    //         left: '0px',              // align to parent left
    //         width: '100%',           // or '100%' or leave it if auto is fine
    //       }); // or a specific value
    //       $dropdown.children('.select2-dropdown').css({width: '100%'});
    ...
    function submit() {
        ...
        formData.delete("currentCountry");
        formData.set("country", formData.get("country").toUpperCase());
        ...
        request.onload = function () {
            if (request.status === 200) {
              ...
              `;
                ...
                rezult.classList.add("rezult--show6sec");
            }
        };
    ...
    let visitorIdInp = document.getElementById("visitorId");
    ...
    fpPromise
        ...
        .then((result) => {
            ...
            visitorIdInp.value = visitorId;
        })
        .catch((error) => console.error(error));

    // показать изначально поле с промокодом
    function showPromocodeInput() {
        promoSwitch.checked = "checked";
        promoInput.className = "promo-show";
        promoInput.disabled = false;
    }

    promoSwitch ? showPromocodeInput() : null;

    // кастомная кнопка для формы пока нет брендбука
    function customFormBtnInit() {
        const text = btnSubmit.textContent;

        btnSubmit.classList.add("pulse-button");
        btnSubmit.classList.add("custom-btn");
        btnSubmit.classList.add("btn-7");
        btnSubmit.innerHTML = `<span>${text}</span>`;
    }

    customFormBtnInit();

    document.forms[0].append(rezult);
});
